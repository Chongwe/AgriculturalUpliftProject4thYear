...
import { postDetailQuery, commentCountQuery, userQuery } from '../utils/data';
...
const CommentPage = () => {
  ...
  const fetchPostData = async () => {
    try {
      ...
      const commentCountQueryResult = await client.fetch(commentCountQuery(postId));
      ...
      <div className="flex flex-col justify-center items-center h-full mt-5 lg:h-4/5">
        <div className="flex lg:flex-row flex-col justify-center rounded-lg bg-green-100 lg:p-5 p-3 lg:w-4/5 w-full">
          <div className=" p-3 flex-0.7 rounded-2xl w-full">
            <img src={post?.image && urlFor(post.image).url()} className="flex rounded-t-2xl rounded-b-sm w-full mt-2" />
          </div>
          ...
          <div className="flex flex-1 flex-col border-l-2 border-goldenrod gap-6 lg:pl-5 mt-5 w-full">
            ...
            <h1 className="text-goldenrod -mb-4 text-2xl">Comments ({commentCount})</h1>
            <hr className="mb-2 border-goldenrod" />
            <div className='max-h-370 overflow-y-auto '>
              {post?.comments?.map((comment, i) => (
                <div key={i} className='flex gap-2 mt-2 rounded-lg items-center'>
                  <img
            ...
            <div className='flex flex-wrap gap-3 '>
              {user && <img src={user.image} className='w-10 h-10 p-2 rounded-full' />}
              <input
