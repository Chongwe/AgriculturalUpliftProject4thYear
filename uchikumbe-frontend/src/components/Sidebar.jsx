 export default function  Sidebar() {
    return (
        <div floated={false} className="h-80 shadow-none"   >
          <div className="container m-2 mx-auto pt-10">
              <h1 className="text-2xl font-bold text-green-900 ml-2 ">Whats new?</h1>
              <p className="border-l-2 p-4 m-3 border-green-800">Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolorem ut magni 
                inventore quidem eius corrupti repudiandae nisi nostrum
                 necessitatibus eligendi praesentium voluptatibus molestiae,
                  aperiam error illo deserunt obcaecati aspernatur itaque.
              </p>
          </div>
        </div>
    );
  }